...
## DONT FORGET TO CHANGE NUM CORES!!
...
sim_name <- paste0("sims", substr(num_sim, 1, nchar(num_sim)-3), "k_cluster181")

# create data.frame with all parameter values ---------------
# sample size
sample_size <- rep(181, num_sim) # equal to smallest pop size, 181 for genetic clusters, 18 for populations (locality)
# number of loci
num_loci <- rep(39, num_sim)
...
create_N <- function(){
  ...
  while (!(nbot < pop_size) & !(nbot < nhist)) {
    ...
    nbot <- round(runif(1, min = 1, max = 600), 0) # original 500
...
# min generation time is about 10 years for the fur seals, hunting ended ca 100 years ago. But as generations overlap we choose a higher max
...
create_t <- function(){
  tbotend <- 1
  tbotstart <- 1
  # duration of bottleneck is at least 2 generations!
  while (!((tbotstart - tbotend) > 2)) {
    tbotend <- round(runif(1, min = 1, max = 20)) #was 1-30
    tbotstart <- round(runif(1, min = 10, max = 40)) #was 10-70
  }
  c(tbotend, tbotstart)
}
...
# Run function on cluster with 35 cores
