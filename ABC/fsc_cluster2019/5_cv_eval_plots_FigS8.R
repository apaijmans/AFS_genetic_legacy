...
load("model_evaluation/check4_params/cv_param_rej_sims10000k_cluster181_bot_3.RData")
...
ggsave(p_all, file = "figs/cv_plots_all.jpg", width = 7, height = 7)


# Ne bot & Ne hist cross-validation plots (just to have a better look, not included in final paper)
...
p_nhist <- cv_params %>%
  ...
  scale_y_continuous(limits = c(0, 200000)) + # see above
  xlab("True value") +
  ylab("Parameter estimate")  
