...
# files needed:
# (1) sumstats for emperical data: all_sumstats_full_all_gaz_8pop.txt (generated in script a+b)
# (2) simulation results are saved as text file sims10000k_pop18.txt
...
sim_name <- paste0("sims10000k_pop18") 

###### Load emperical data ###### 

# Read in data
all_sumstats_full <- read.table("all_sumstats_full_all_gaz_8pop.txt", header = T, row.names=1, stringsAsFactors = F)
...
all_probs_mar <- abc::postpr(target = all_sumstats["mar",], index = models, sumstat = sims_stats, tol = tol, method = method)
all_probs_ci <- abc::postpr(target = all_sumstats["ci",], index = models, sumstat = sims_stats, tol = tol, method = method)
all_probs_ki <- abc::postpr(target = all_sumstats["ki",], index = models, sumstat = sims_stats, tol = tol, method = method)
all_probs_hi <- abc::postpr(target = all_sumstats["hi",], index = models, sumstat = sims_stats, tol = tol, method = method)
all_probs_mac <- abc::postpr(target = all_sumstats["mac",], index = models, sumstat = sims_stats, tol = tol, method = method)
...
out4 <- round(summary(all_probs_mar)$Prob, 3)
out5 <- round(summary(all_probs_ci)$Prob, 3)
out6 <- round(summary(all_probs_ki)$Prob, 3)
out7 <- round(summary(all_probs_hi)$Prob, 3)
out8 <- round(summary(all_probs_mac)$Prob, 3)
...
all_fits_neut3 <- abc::gfit(target = all_sumstats["bi",], sumstat = sims_stats,
                            nb.replicate = cv_rep, tol = tol, subset = models == "neut")
all_fits_neut4 <- abc::gfit(target = all_sumstats["mar",], sumstat = sims_stats,
                            nb.replicate = cv_rep, tol = tol, subset = models == "neut")
all_fits_neut5 <- abc::gfit(target = all_sumstats["ci",], sumstat = sims_stats,
                            nb.replicate = cv_rep, tol = tol, subset = models == "neut")
all_fits_neut6 <- abc::gfit(target = all_sumstats["ki",], sumstat = sims_stats,
                            nb.replicate = cv_rep, tol = tol, subset = models == "neut")
all_fits_neut7 <- abc::gfit(target = all_sumstats["hi",], sumstat = sims_stats,
                            nb.replicate = cv_rep, tol = tol, subset = models == "neut")
all_fits_neut8 <- abc::gfit(target = all_sumstats["mac",], sumstat = sims_stats,
                            nb.replicate = cv_rep, tol = tol, subset = models == "neut")
...
p6 <- summary(all_fits_neut6)$pvalue
p7 <- summary(all_fits_neut7)$pvalue
p8 <- summary(all_fits_neut8)$pvalue


p_neut <- rbind(p1, p2, p3, p4, p5, p6, p7, p8)
...
all_fits_bot3 <- abc::gfit(target = all_sumstats["bi",], sumstat = sims_stats,
                           nb.replicate = cv_rep, tol = tol, subset = models == "bot")
all_fits_bot4 <- abc::gfit(target = all_sumstats["mar",], sumstat = sims_stats,
                           nb.replicate = cv_rep, tol = tol, subset = models == "bot")
all_fits_bot5 <- abc::gfit(target = all_sumstats["ci",], sumstat = sims_stats,
                           nb.replicate = cv_rep, tol = tol, subset = models == "bot")
all_fits_bot6 <- abc::gfit(target = all_sumstats["ki",], sumstat = sims_stats,
                           nb.replicate = cv_rep, tol = tol, subset = models == "bot")
all_fits_bot7 <- abc::gfit(target = all_sumstats["hi",], sumstat = sims_stats,
                           nb.replicate = cv_rep, tol = tol, subset = models == "bot")
all_fits_bot8 <- abc::gfit(target = all_sumstats["mac",], sumstat = sims_stats,
                           nb.replicate = cv_rep, tol = tol, subset = models == "bot")
...
pb6 <- summary(all_fits_bot6)$pvalue
pb7 <- summary(all_fits_bot7)$pvalue
pb8 <- summary(all_fits_bot8)$pvalue

# combine into table
p_bot <- rbind(pb1, pb2, pb3, pb4, pb5, pb6, pb7, pb8)
