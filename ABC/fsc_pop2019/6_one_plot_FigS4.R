...
# Read in data
all_sumstats_full <- read.table("all_sumstats_8pops_full_1000_10indv.txt", header = T, row.names=1, stringsAsFactors = F)
all_sumstats_full$pop <- gsub("MAQ", "MAC", all_sumstats_full$pop, fixed = TRUE)

# Select summary statistics that we want to plot
...
load("abc_estimates/abc_sims10000k_pop18_bot_ALL_3.RData")
...
prophet <- read.table("BOTTLENECK_1-1000_pop_long.txt", header = T, stringsAsFactors = F)
...
allstats_long$pop <- ordered(allstats_long$pop, levels=c("SSI", "SG", "BI", "MAR", "CI", "KI", "HI", "MAC"))
...
ggplot2::ggsave(filename = "figs/Ar_Hobs_prophetexc_and_nbot_pop.jpg", p, width = 8, height = 6)
