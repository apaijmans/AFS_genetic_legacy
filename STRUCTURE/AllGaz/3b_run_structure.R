...
# Usage: nohup Rscript 3b_run_structure.R &

options(scipen=999)

# Hier

gen <- fread("structure_in_gaz.stru")

#~~ Specify in and out files for structure

infile <- "structure_in_gaz.stru"
outpath <- "results/"
...
#~~ make the job matrix
pop <- "1,2,3,4,5,6,7,8" #number of pops in the file
...
ParallelStructure::parallel_structure(structure_path=STR_path, joblist='seal_jobs.txt', n_cpu=45, 
              infile='structure_in_gaz.stru', 
              outpath='results/', 
              numinds = nrow(gen)/2, 
              numloci=ncol(gen)-2,
